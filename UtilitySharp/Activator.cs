...
namespace UtilitySharp
{
    class Activator
    {
        ...
        public Activator(Menu mainMenu)
        {
            ...
            if (HEAL.Slot != SpellSlot.Unknown)
            {
                ...
                HealTeamatePercent = menu.Add("HealTeamatePercent",
                    new Slider("Use at Teamate Health %:{0}", 30, 1, 99));
            }
            if (BARRIER.Slot != SpellSlot.Unknown)
            {
            ...
            if (IGNITE.Slot != SpellSlot.Unknown)
            {
                ...
                IgniteKSEnable = menu.Add("IgniteKSEnable", new CheckBox("Enable Ignite"));
            }
        ...
        private void LoadSpells()
        {
            ...
            SMITE = new Spell.Targeted(Hero.GetSpellSlotFromName("summonersmite"), 500);
            if (SMITE.Slot == SpellSlot.Unknown) { SMITE = new Spell.Targeted(Hero.GetSpellSlotFromName("itemsmiteaoe"), 500); }
            if (SMITE.Slot == SpellSlot.Unknown) { SMITE = new Spell.Targeted(Hero.GetSpellSlotFromName("s5_summonersmiteplayerganker"), 500); }
            if (SMITE.Slot == SpellSlot.Unknown) { SMITE = new Spell.Targeted(Hero.GetSpellSlotFromName("s5_summonersmitequick"), 500); }
            if (SMITE.Slot == SpellSlot.Unknown) { SMITE = new Spell.Targeted(Hero.GetSpellSlotFromName("s5_summonersmiteduel"), 500); }
        ...
        public int GetCustomIgniteDamage()
        {
        ...
        private void UseDefensives()
        {
            ...
            //HEAL
            if (HEAL.Slot != SpellSlot.Unknown && HEAL.IsReady() && HealActive.CurrentValue && !(Hero.IsDead))
            {
            ...
            //BARRIER
            if (BARRIER.Slot != SpellSlot.Unknown && BARRIER.IsReady() && BarrierActive.CurrentValue && !(Hero.IsDead) &&
        ...
        {
            if (MenuGUI.IsChatOpen || Chat.IsOpen || SMITE.Slot == SpellSlot.Unknown)
            {
                return;
            }
        ...
        {
            if (SMITE.Slot != SpellSlot.Unknown && SMITE.IsReady() && SmiteActive.CurrentValue)
            {   
                foreach (var minion in EntityManager.MinionsAndMonsters.GetJungleMonsters().Where(x => x.IsValidTarget(600)))
                {                               
                    if (minion != null && minion.Health <= GetSmiteDamage(Hero.Level))
                    {
        ...
        private void Combo()
        {
            ...
            //Smite Enemies
            if (SMITE.Slot != SpellSlot.Unknown && SMITE.IsReady() && SmiteEnemies.CurrentValue)
            {
            ...
            //IGNITE
            if (IGNITE.Slot != SpellSlot.Unknown &&  IgniteKSEnable.CurrentValue)
            {
                if (IGNITE.IsReady())
                {
        ...
        {
            if (SMITE.Slot != SpellSlot.Unknown && DrawSmiteEnabled.CurrentValue)
            {
